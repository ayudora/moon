window.ThreeViewer = (function () {
  ...
  function init3D() {
    ...
    [
      { x: 100, y: 100, rx: 60, ry: 35 },
      { x: 170, y: 130, rx: 35, ry: 25 },
      { x: 320, y: 100, rx: 50, ry: 40 },
      { x: 340, y: 180, rx: 30, ry: 45 },
      { x: 220, y: 170, rx: 35, ry: 35 },
      { x: 440, y: 160, rx: 40, ry: 25 },
    ].forEach((c) => {
      eCtx.beginPath();
      eCtx.ellipse(c.x, c.y, c.rx, c.ry, 0, 0, Math.PI * 2);
      eCtx.fill();
    });
    ...
    const gMoonMatBody = new THREE.MeshPhongMaterial({
      color: 0xdddddd,
      emissive: 0x111111,
      shininess: 5,
    });
    ...
    labelE = createLabelSprite(
      "東(昇る)",
      "rgba(15, 23, 42, 0.8)",
      "#93c5fd",
      "rgba(147, 197, 253, 0.4)",
    );
    labelE.position.set(-54, 2, 0);
    groundGroup.add(labelE);
    labelW = createLabelSprite(
      "西(沈む)",
      "rgba(15, 23, 42, 0.8)",
      "#fca5a5",
      "rgba(252, 165, 165, 0.4)",
    );
    labelW.position.set(54, 2, 0);
    groundGroup.add(labelW);
    labelN = createLabelSprite(
      "北(背後)",
      "rgba(15, 23, 42, 0.8)",
      "#d1d5db",
      "rgba(209, 213, 219, 0.4)",
    );
    labelN.position.set(0, 2, 54);
    groundGroup.add(labelN);

    labelZenith = createLabelSprite(
      "天頂(真上)",
      "rgba(15, 23, 42, 0.8)",
      "#c084fc",
      "rgba(192, 132, 252, 0.4)",
    );
    labelZenith.position.set(0, 48, -10);
    groundGroup.add(labelZenith);
  ...
  function handleCameraDrag(deltaX, deltaY) {
    ...
    if (state.viewMode === "space") {
      ...
      camElevation = Math.max(
        -1.3,
        Math.min(1.48, camElevation + deltaY * 0.008),
      );
      ...
      if (state.groundMode === "firstPerson") {
        ...
        gCamPitch = Math.max(
          -Math.PI / 2 + 0.1,
          Math.min(Math.PI / 2 - 0.1, gCamPitch - deltaY * 0.005),
        );
  ...
  function applyCameraTransform() {
    if (state.viewMode === "space") {
      ...
      if (isAnimatingToPreset) {
        ...
        if (
          Math.abs(targetCamTheta - camTheta) < 0.001 &&
          Math.abs(targetCamElevation - camElevation) < 0.001 &&
          Math.abs(targetCamRadius - camRadius) < 0.01
        ) {
          isAnimatingToPreset = false;
        }
      }
      ...
      if (state.groundMode === "overview") {
        ...
        if (isAnimatingToPreset) {
          ...
          if (
            Math.abs(targetGCamYaw - gCamYaw) < 0.001 &&
            Math.abs(targetGCamPitch - gCamPitch) < 0.001 &&
            Math.abs(targetCamFov - camFov) < 0.1
          ) {
            isAnimatingToPreset = false;
          }
        }
  ...
  function setViewMode(mode) {
    ...
    if (mode === "space") {
      btnSpace.className =
        "px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition bg-indigo-600 text-white flex items-center gap-1.5 shadow";
      btnGround.className =
        "px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition text-slate-400 hover:text-white flex items-center gap-1.5 hover:bg-slate-800";

      universeGroup.visible = true;
      groundGroup.visible = false;

      presetSpace.classList.remove("hidden");
      presetSpace.classList.add("flex");
      subModeToggle.classList.add("hidden");
      subModeToggle.classList.remove("flex");
      presetGround.classList.add("hidden");
      presetGround.classList.remove("flex");

      hintLabel.innerHTML =
        '<i class="fa-solid fa-arrows-up-down-left-right text-indigo-400 text-sm"></i><span class="text-xs sm:text-sm font-bold text-slate-200">見回す</span>';
      ...
      btnSpace.className =
        "px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition text-slate-400 hover:text-white flex items-center gap-1.5 hover:bg-slate-800";
      btnGround.className =
        "px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition bg-emerald-600 text-white flex items-center gap-1.5 shadow";

      universeGroup.visible = false;
      groundGroup.visible = true;

      presetSpace.classList.add("hidden");
      presetSpace.classList.remove("flex");
      subModeToggle.classList.remove("hidden");
      subModeToggle.classList.add("flex");

      setGroundMode(
        state.groundMode === "overview" ? "overview" : "firstPerson",
      );
  ...
  function setGroundMode(mode) {
    ...
    if (mode === "firstPerson") {
      ...
      presetGround.classList.remove("hidden");
      presetGround.classList.add("flex");
      ...
      hintLabel.innerHTML =
        '<i class="fa-solid fa-arrows-up-down-left-right text-emerald-400 text-sm"></i><span class="text-xs sm:text-sm font-bold text-slate-200">見回す</span>';
      hintLabel.style.display = "flex";
      ...
      btnFP.className =
        "px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition text-slate-400 hover:text-white flex items-center gap-1.5 hover:bg-slate-800";
      btnOV.className =
        "px-3 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition bg-fuchsia-600 text-white flex items-center gap-1.5 shadow";
  ...
  function setPresetView(type) {
    ...
    if (state.orbitMode === "sunMoves") {
      ...
      if (type === "sun-left") {
        targetCamTheta = Math.PI / 2;
        targetCamElevation = 0.35;
        targetCamRadius = 55;
      } else if (type === "sun-right") {
        targetCamTheta = -Math.PI / 2;
        targetCamElevation = 0.35;
        targetCamRadius = 55;
      } else if (type === "top-left") {
        targetCamTheta = Math.PI / 2;
        targetCamElevation = 1.45;
        targetCamRadius = 55;
  ...
  function setGroundPreset(dir) {
    ...
    if (dir === "south") {
      ...
      targetGCamYaw = Math.PI / 2;
      targetGCamPitch = 0.4;
    } else if (dir === "west") {
      targetGCamYaw = -Math.PI / 2;
      targetGCamPitch = 0.4;
    } else if (dir === "zenith") {
      targetGCamPitch = Math.PI / 2 - 0.2;
    }
  }
  ...
  function update3DPositions() {
    ...
    if (groundGroup && groundGroup.visible) {
      ...
      const my =
        R_MOON * Math.cos(moonHourAngle) * Math.sin((55 * Math.PI) / 180);
      const mz =
        -R_MOON * Math.cos(moonHourAngle) * Math.cos((55 * Math.PI) / 180);
      ...
      if (state.groundMode === "overview") {
        ...
        const V = new THREE.Vector3()
          .subVectors(ovCamPos, gMoonMesh.position)
          .normalize();
        ...
        gSunLight.position
          .copy(gMoonMesh.position)
          .addScaledVector(sunDir, 100);
        gSunLight.target.position.copy(gMoonMesh.position);
      }
  ...
  function animate3D() {
    ...
    // --- ここから追加 ---
    // 「観察」タブ以外（図鑑やクイズ）を開いている時は、3Dの計算をお休みして動作を軽くする
    const observeView = document.getElementById("view-observe");
    if (observeView && observeView.classList.contains("hidden")) {
      return;
    }
    // --- ここまで追加 ---
