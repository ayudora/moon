...
const MoonCanvas = {
  draw: function () {
    ...
    if (isNewMoon) {
      ...
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(cx, cy, bulgeWidth, r, 0, -Math.PI / 2, Math.PI / 2, false);
      ctx.fillStyle = darkColor;
      ctx.fill();
      ...
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(cx, cy, bulgeWidth, r, 0, -Math.PI / 2, Math.PI / 2, true);
      ctx.fillStyle = litGrad;
      ctx.fill();
    ...
    ctx.arc(cx - 10, cy + 30, 12, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  },
};
...
function updateSkyBackground() {
  ...
  const baseClasses =
    "w-48 h-48 md:w-56 md:h-56 rounded-full border-4 border-slate-800 flex items-center justify-center relative shadow-2xl overflow-hidden transition-colors duration-500";

  if (h >= 6 && h < 16) {
    skyBg.className =
      baseClasses + " bg-gradient-to-b from-slate-500 to-slate-600";
  } else if ((h >= 16 && h < 19) || (h >= 4 && h < 6)) {
    skyBg.className =
      baseClasses +
      " bg-gradient-to-b from-slate-800 via-stone-700 to-orange-900";
  } else {
...
function updateAllViews() {
  ...
  document.getElementById("orbitDaysDisplay").innerText =
    state.moonAge.toFixed(1);
  ...
  document.getElementById("timeNameVal").innerHTML = getTimeNameHtml(
    state.timeHour,
  );
  ...
  document.getElementById("explanationTitle").innerText =
    `${phase.name} の特徴`;
...
function togglePlay() {
  ...
  [
    ...
    { b: "fsPlayBtn", i: "fsPlayIcon", t: "fsPlayText" },
  ].forEach((ids) => {
...
function togglePlayEarth() {
  ...
  [
    ...
    { b: "fsPlayEarthBtn", i: "fsPlayEarthIcon", t: "fsPlayEarthText" },
  ].forEach((ids) => {
    const btn = document.getElementById(ids.b);
    const icon = document.getElementById(ids.i);
    const text = document.getElementById(ids.t);
...
function setSpeed(spd) {
  ...
  ["025", "05", "10", "20"].forEach((id) => {
    ...
    const defaultClass =
      "px-2 py-1 rounded transition text-slate-400 hover:text-white";
    if (el1) el1.className = defaultClass;
    if (el2) el2.className = defaultClass;
  });
  const activeId =
    spd === 0.25 ? "025" : spd === 0.5 ? "05" : spd === 1.0 ? "10" : "20";
  const activeClass =
    "px-2 py-1 rounded transition bg-slate-800 text-amber-300 font-bold";
...
function switchTab(tab) {
  ...
  if (tab === "observe" && window.ThreeViewer)
    setTimeout(ThreeViewer.onWindowResize, 100);
}
...
function setOrbitMode(mode) {
  ...
  const activeClass =
    "px-5 py-2 rounded-lg bg-slate-700 text-white font-bold transition shadow";
  const activeClassAmber =
    "px-5 py-2 rounded-lg bg-amber-600 text-white font-bold transition shadow";
  const inactiveClass =
    "px-5 py-2 rounded-lg text-slate-400 hover:text-white transition";
...
const GalleryManager = {
  render: function () {
    ...
    MOON_PHASES.forEach((phase) => {
      ...
      card.className =
        "bg-slate-950/80 border border-slate-800 rounded-xl p-4 flex flex-col justify-between hover:border-amber-400/50 transition duration-300 shadow-lg group cursor-pointer";
      card.onclick = () => {
...
const QuizManager = {
  ...
  showQuestion: function () {
    ...
    document.getElementById("quizQuestionNum").innerText =
      `第 ${this.currentIdx + 1} 問 / 全 5 問`;
    ...
    q.options.forEach((opt, idx) => {
      ...
      btn.className =
        "bg-slate-950 hover:bg-slate-800 border border-slate-800 text-left p-3.5 rounded-xl font-bold text-xs sm:text-sm text-slate-200 transition flex items-center justify-between shadow";
  ...
  selectAnswer: function (selectedIdx) {
    ...
    if (isCorrect) {
      resultHeader.className =
        "font-black text-lg text-emerald-400 flex items-center gap-2";
      resultHeader.innerHTML = `<i class="fa-solid fa-circle-check"></i> 大正解！すごい！`;
    } else {
      resultHeader.className =
        "font-black text-lg text-rose-400 flex items-center gap-2";
    ...
    Array.from(document.getElementById("quizOptions").children).forEach(
      (btn, idx) => {
        ...
        if (idx === q.correct)
          btn.className =
            "bg-emerald-950/80 border-2 border-emerald-500 text-left p-3.5 rounded-xl font-bold text-xs sm:text-sm text-emerald-300 flex items-center justify-between shadow";
        else if (idx === selectedIdx && !isCorrect)
          btn.className =
            "bg-rose-950/80 border-2 border-rose-500 text-left p-3.5 rounded-xl font-bold text-xs sm:text-sm text-rose-300 flex items-center justify-between shadow";
        else
          btn.className =
            "opacity-40 bg-slate-950 border border-slate-800 text-left p-3.5 rounded-xl font-bold text-xs sm:text-sm text-slate-400 flex items-center justify-between";
      },
    );
  },
...
const FullscreenManager = {
  ...
  syncUi: function (elementId, isFull, sourceButton = null) {
    document
      .querySelectorAll(`[data-fullscreen-button="${elementId}"]`)
      .forEach((btn) => {
